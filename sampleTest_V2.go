...
func printGraph(myVertices *map[string]Node){
fmt.Println("-------------Printing te Graph-----------------")
fmt.Println("Creating Nodes in Graph")

//Node Parameters

mapLength := len((*myVertices))
currentIndex := 0
createNode := "Create "
createNodeDelimiter := ""
...
for parentKey := range(*myVertices){
        parentNodeType := ""
        currentIndex = currentIndex + 1
        propertyType := "name:"
        if (strings.Contains(parentKey,".sql")){
                parentNodeType = "Script"
                } else {
        ...
        for childKey := range((*myVertices)[parentKey].childL){
                ...
                relation := "Create "
                childNodeType := ""
                relationProperty := ":Depends_On"
                if (strings.Contains(childKey,".sql")){
                childNodeType = "Script"
                } else {
                        childNodeType = "Table"
                        }
                matchStatement = matchStatement + "(pNode:" + parentNodeType +"), (cNode:" + childNodeType + ") "
                condition = condition + "pNode.name=" + "\"" + parentKey  +"\"" + " and " + "cNode.name=" + "\"" + childKey + "\" "
                relation = relation + "(pNode) - [" + relationProperty + "] -> (cNode)"
                createChildRelationship = createChildRelationship + matchStatement + condition + relation + ";\n"
                }
        createNode = createNode + " (:"+parentNodeType + "{" +propertyType + "\"" + parentKey + "\"" + "}" +") " + createNodeDelimiter
        }
fmt.Println(createNode)
fmt.Println(createChildRelationship)
